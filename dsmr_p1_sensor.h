...
class CustomP1UartComponent : public Component, public uart::UARTDevice {
 ...
 public:
  ...
  void setup() override {
    lastread = 0;
	pinMode(4, OUTPUT); // Set D5 as output pin
	digitalWrite(4,LOW); // Set low, don't request message from P1 port
  }
  ...
  void loop() override {
	...
	if (now - lastread > DELAY_MS || lastread == 0) {
		lastread = now;
		digitalWrite(4,HIGH); // Set high, request new message from P1 port
		if (data_available()) { // Check for x seconds if there's data available
			bool have_message = read_message();
			if (have_message) { 
				digitalWrite(4,LOW); // Set low, stop requesting messages from P1 port
